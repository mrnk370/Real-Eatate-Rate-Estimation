...
def index():
    ...
    bhks = [1, 2, 3, 4, 5]
    return render_template('index.html', area_types=area_types, locations=locations, bhks=bhks)
...
def compare_page():
    locations = sorted([loc['name'] for loc in location_data])
    bhks = [1, 2, 3, 4, 5]
    return render_template('compare_from.html', locations=locations, bhks=bhks)
...
def compare():
    try:
        ...
        return render_template('compare_result.html', p1=p1, p2=p2)
